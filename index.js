...
photoHandler.on('photo', async (ctx) => {
    let res = await ctx.telegram.getFile(ctx.message.photo[ctx.message.photo.length - 1].file_id)
    ctx.session.img = "https://api.telegram.org/file/bot" + "1954554074:AAE4j3DpZ2ll5OqWw-mQxkCOHd1TgEx5wwQ" + "/" + res.file_path
    ctx.reply('Аризангиз юборилди', Markup.inlineKeyboard(
        [
    ...
    pdf
        ...
        .then(() => {
            ctx.telegram.sendDocument(-1001237984705, {'source': "./anketa/" + ctx.from.id + ".pdf"}, {
                "caption": "<b>Yangi zayavka!</b>\n\n Foydalanuvchi <a href=\"tg://user?id=" + ctx.from.id + "\">" + ctx.from.id + "</a>",
                "parse_mode": "html"
            })
        })
        .catch((error) => {
...
malumotHandler.on('callback_query', async (ctx) => {
    ...
    ctx.reply('Оилавий аxволингиз?', Markup.inlineKeyboard([
...
telefon.on('text', (ctx) => {
    ...
    ctx.reply('Маълумотингиз?', Markup.inlineKeyboard([
...
bizda_maosh.on('text', (ctx) => {
    ...
    ctx.reply('Кайси йуналишда (оддий ходим, сотув, бошкарув, хисоб китоб, техник, слесарь, сварщик,бухгалтерия) бемалол кийналмай ишлай оласиз?\n')
    return ctx.wizard.next()
})
...
about_me.on('text', (ctx) => {
    ...
    ctx.reply('Расмингизни жонатинг (3x4 йоки селфи тарикасида)')
    return ctx.wizard.next()
})

const infoScene = new WizardScene('infoScene',
    (ctx) => {
...
infoScene.start(ctx => {
    ctx.scene.leave()
    ctx.replyWithHTML("<b>Assalomu Aleykum</b> " + ctx.from.first_name + " <b>Elit Decor HR botiga Xush Kelibsiz!</b>", Markup.inlineKeyboard(
        [
            Markup.callbackButton('Biz haqimizda', 'about'),
            Markup.callbackButton("Anketa to’ldirish", 'anketa')
        ]
